...
这个仓库做的是一件比较具体的事：给一段中文 AI 初稿找出“AI 味”来自哪里，再把这些问题整理成改写提示，最后生成一份修改前后的对比报告。

我没有把它做成“同义词替换器”。那种做法看起来自动，实际很难解释为什么分数会降，也容易把文章改得更怪。这里的思路是把流程拆开：先检测，再定位问题，再引入来源材料和作者补充信息，最后复查。
...
仓库里的 `examples/original.md` 是一段故意写得比较模板化的中文 AI 初稿。`examples/revised.md` 则加入了更具体的项目文件、脚本分工、取舍和限制说明。
...
生成的报告不只给一个百分比，还会列出原文命中的模式、高风险句子、各类特征变化和原始 JSON 结果。完整示例在 `examples/report.md`。
...
| `examples/revised.md` | 按提示修改后的版本 |
| `examples/report.md` | 修改前后的对比报告 |
...
5. 运行 `python scripts/run_pipeline.py`。
6. 根据 `examples/rewrite_prompt.md` 改写，保存到 `examples/revised.md`。
7. 再运行 `python scripts/make_report.py ...` 生成对比报告。
